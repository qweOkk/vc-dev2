...
from attacks.utils import resolve_loss
from adaptive_attack.NES import NES
from adaptive_attack.EOT import EOT, EOTSV
import torch
import numpy as np
...
class FAKEBOB(Attack):
    ...
    def __init__(self, model, threshold=None,
                 ...
                 batch_size=1, EOT_size=1, EOT_batch_size=1, verbose=1, **kwargs):
    ...
    def attack_batch(self, x_batch, y_batch, lower, upper, batch_id):
        ...
        with torch.no_grad():
            ...
            for iter in range(self.max_iter + 1):
                ...
                if self.verbose:
                    self.logger.info("batch: {} iter: {}, loss: {}, y: {}, y_pred: {}, best loss: {}".format(
                        batch_id, iter,
    ...
    def attack_batch_sv(self, x1_batch, x2_batch, y_batch, lower, upper, batch_id):

        with torch.no_grad():

            n_audios, _, _ = x2_batch.shape

            last_ls = [[]] * n_audios
            lr = [self.max_lr] * n_audios
            prev_loss = [np.infty] * n_audios

            adver_x1 = x1_batch.clone()
            adver_x2 = x2_batch.clone()
            grad = torch.zeros_like(x2_batch, dtype=x2_batch.dtype, device=x2_batch.device)

            best_adver_x = adver_x2.clone()
            best_loss = [np.infty] * n_audios
            consider_index = list(range(n_audios))
            ...
            for iter in range(self.max_iter + 1):
                prev_grad = grad.clone()
                # loss, grad, adver_loss, scores = self.get_grad(adver_x, y_batch)
                loss, grad, adver_loss, _, y_pred = self.get_grad(adver_x1, adver_x2, y_batch)
                # y_pred = torch.max(scores, 1)[1].cpu().numpy()
                ...
                for ii, adver_l in enumerate(adver_loss):
                    ...
                    if adver_l < best_loss[index]:
                        best_loss[index] = adver_l.cpu().item()
                        best_adver_x[index] = adver_x2[ii]
                ...
                # delete alrady found examples
                adver_x2, y_batch, prev_grad, grad, lower, upper, \
                    consider_index, \
                    last_ls, lr, prev_loss, loss = self.delete_found(adver_loss, adver_x2, y_batch, prev_grad, grad,
                                                                     lower, upper,
                                                                     consider_index, last_ls, lr, prev_loss, loss)
                if adver_x2 is None:  # all found
                    break
                ...
                if iter < self.max_iter:
                    ...
                    lr_t = torch.tensor(lr, device=adver_x2.device, dtype=torch.float).unsqueeze(1).unsqueeze(2)
                    adver_x2.data = adver_x2 + self.grad_sign * lr_t * torch.sign(grad)
                    adver_x2.data = torch.min(torch.max(adver_x2.data, lower), upper)
                    ...
                    if self.stop_early and iter % self.stop_early_iter == 0:
                        ...
                        adver_x2, y_batch, prev_grad, grad, lower, upper, \
                            consider_index, \
                            last_ls, lr, prev_loss, loss = self.delete_found(converge_loss, adver_x2, y_batch, prev_grad,
                                                                             grad, lower, upper,
                                                                             consider_index, last_ls, lr, prev_loss,
                                                                             loss)
                        if adver_x2 is None:  # all converage
                            break
    ...
    def attack_sv(self, x1, x2, y):
        ...
        self.loss, self.grad_sign = resolve_loss(
            loss_name='Margin',
        ...
        self.EOT_wrapper = EOTSV(self.model, self.loss, self.EOT_size, self.EOT_batch_size, False)
        ...
        lower = -1
        upper = 1
        assert lower <= x1.max() < upper, 'generating adversarial examples should be done in [-1, 1) float domain'
        assert lower <= x2.max() < upper, 'generating adversarial examples should be done in [-1, 1) float domain'
        n_audios, n_channels, _ = x2.size()
        assert n_channels == 1, 'Only Support Mono Audio'
        assert y.shape[0] == n_audios, 'The number of x and y should be equal'
        upper = torch.clamp(x2 + self.epsilon, max=upper)
        lower = torch.clamp(x2 - self.epsilon, min=lower)
        ...
        for batch_id in range(n_batches):
            ...
            lower_batch = lower[batch_id * batch_size:(batch_id + 1) * batch_size]
            upper_batch = upper[batch_id * batch_size:(batch_id + 1) * batch_size]
            adver_x_batch, success_batch = self.attack_batch_sv(x1_batch, x2_batch, y_batch, lower_batch, upper_batch,
                                                                batch_id)
            if batch_id == 0:
                adver_x = adver_x_batch
                success = success_batch
    ...
    def estimate_threshold_run(self, x1, x2, step=0.1):
        n_audios, _, _ = x1.shape
        d, s = self.model.make_decision_SV(x1, x2)
        d = d[0]
        s = s[0]
        if d != 0:
            return  # aleady accept, cannot be used to estimate threshold
        y = torch.tensor([0] * n_audios, dtype=torch.long, device=x2.device)
        ...
        adver_x1 = x1.clone()
        adver_x2 = x2.clone()
        grad = torch.zeros_like(x2, dtype=x2.dtype, device=x2.device)
        ...
        while True:
            ...
            while True:
                ...
                decision, score = self.model.make_decision_SV(adver_x1, adver_x2)
                decision = decision[0]
                score = score[0]
                score = np.max(score.cpu().numpy())
                self.logger.info(
                    'iter outer: {}, inter_inner: {}, score: {} model_threshold:{}'.format(iter_outer, iter_inner,
                                                                                           score,
                                                                                           self.model.threshold))
                if decision != 0:  # succeed, found the threshold
                    return score
                ...
                # not succeed, update
                prev_grad = grad.clone()
                loss, grad, _, _, _ = self.get_grad(adver_x1, adver_x2, y)
    ...
    def estimate_threshold(self, x1, x2, step=0.1):
        if self.task == 'CSI':
            self.logger.info("--- Warning: no need to estimate threshold for CSI, quitting ---")
            return

        with torch.no_grad():
            estimated_thresholds = []
            for xx1, xx2 in zip(x1.unsqueeze(0), x2.unsqueeze(0)):  # parallel running, not easy for batch running
                estimated_threshold = self.estimate_threshold_run(xx1, xx2, step)
