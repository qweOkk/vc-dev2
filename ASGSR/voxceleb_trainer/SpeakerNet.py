...
class ModelTrainer(object):
    ...
    def evaluateFromList(self, test_list, test_path, nDataLoaderThread, distributed, print_interval=100, num_eval=10, score_type='cos_sim', **kwargs):
        ...
        if rank == 0:
            ...
            for idx, line in enumerate(lines):
                ...
                if score_type == 'cos_sim':
                    cos = F.cosine_similarity(ref_feat.reshape(num_eval, -1), com_feat.reshape(num_eval, -1)).detach().cpu().numpy()
                    score = numpy.mean(cos)

                elif score_type == 'euc_dist':
                    dist = torch.cdist(ref_feat.reshape(num_eval, -1), com_feat.reshape(num_eval, -1)).detach().cpu().numpy()
                    score = -1 * numpy.mean(dist)  # flip sign to make it a similarity score
