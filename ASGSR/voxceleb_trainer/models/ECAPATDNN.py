...
if __name__ == "__main__":
    ...
    ## model to pth
    load_stat = torch.load('/home/wangli/ASGSR/pretrained_models/ECAPATDNN/ECAPATDNN.model', map_location='cpu')
    new_stat = OrderedDict()
    # choose the keys which have "speaer_encoder"
    for k, v in load_stat.items():
        if "speaker_encoder" in k:
            new_stat["__S__." + k[len('speaker_encoder.'):]] = v
    # model = MainModel(C=1024)
    # model.load_state_dict(new_stat)
    # print(model.state_dict())
    # model.eval()
    # torch.save(model.state_dict(), '/home/wangli/ASGSR/pretrained_models/ECAPATDNN.pth')
    torch.save(new_stat, '/home/wangli/ASGSR/pretrained_models/ECAPATDNN/ECAPATDNN.pth')
