...
sys.path.append('../')
from attacks.utils import resolve_prediction
import torch
import torch.nn as nn


# 黑盒查询方法
#   原理：一条音频，复制N份，每份加一个随机噪声，得到N条略有不同的音频，然后用这N条音频去预测
#   得到N个预测结果，然后根据这N个预测结果，分数越高的随机噪声越有可能是对抗样本
#   对噪声加权求和，得到近似梯度
#   N需要很大，否则梯度不准确
#   FakeBob N=50
class NES(nn.Module):
    ...
    def forward(self, x1, x2, y):
        n_audios, n_channels, N = x2.shape
        num_batches = self.samples_per_draw // self.samples_per_draw_batch_size
        for i in range(num_batches):
            noise = torch.randn([n_audios, self.samples_per_draw_batch_size // 2, n_channels, N], device=x2.device)
            ...
            if i == 0:
                noise = torch.cat((torch.zeros_like(x2, device=x2.device).unsqueeze(1), noise), 1)
            eval_input = noise * self.sigma + x2.unsqueeze(1)
            ...
            for jj, y_ in enumerate(y):
                tmp = torch.tensor(
                    ...
                    dtype=torch.long, device=x2.device)
                if jj == 0:
                    eval_y = tmp
                else:
            ...
            scores, loss, _, decisions = self.EOT_wrapper(x1, eval_input, eval_y)
