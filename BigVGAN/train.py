...
def train(rank, a, h):
    ...
    print("Generator params: {} M".format(sum(p.numel() for p in generator.parameters()) / 1000000 ))

    # define discriminators. MPD is used by default
    mpd = MultiPeriodDiscriminator(h).to(device)
    print("Discriminator mpd params: {}".format(sum(p.numel() for p in mpd.parameters()) / 1000000 ))

    # define additional discriminators. BigVGAN uses MRD as default
    mrd = MultiResolutionDiscriminator(h).to(device)
    print("Discriminator mrd params: {}".format(sum(p.numel() for p in mrd.parameters()) / 1000000 ))
    ...
    train_steps = len(trainset) // h.batch_size // h.num_gpus
    ...
    if rank == 0:
        ...
        testset = MelDataset(test_filelist, h, h.segment_size, h.n_fft, h.num_mels,
                              h.hop_size, h.win_size, h.sampling_rate, h.fmin, h.fmax, False, False, n_cache_reuse=0,
                              fmax_loss=h.fmax_for_loss, device=device, fine_tuning=a.fine_tuning)
        ...
        for i, batch in enumerate(train_loader):
            ...
            if rank == 0:
                ...
                if steps % a.stdout_interval == 0:
                    ...
                    # reserve 2-point
                    logger.info("[Epoch: {}, Step: {}/{}], Gen: {:.2f}, Disc: {:.2f}, Mel-Spec. Error: {:.2f}, s/b: {:.2f}".format(
                        epoch, i, train_steps, loss_gen_all, loss_disc_all, mel_error, time.time() - start_b))
...
def main():
    ...
    parser.add_argument('--input_test_file', default="LibriTTS/test-clean.txt")
    ...
    parser.add_argument('--summary_interval', default=10000000, type=int)
    parser.add_argument('--validation_interval', default=10000000, type=int)
